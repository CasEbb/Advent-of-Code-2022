...
faces = {}
for i, j in visited:
    faces[(i // face_size, j // face_size)] = [
        ((v[0] // face_size, v[1] // face_size) if v is not None else v)
        for v in visited[(i, j)]
    ]
    ...
    if step == "R":
        ...
        for i in range(step):
            ...
            if grid[nx][ny] == " ":
                ...
                while tuple(
                    [
                        a * face_size + b + c * cur_offset
                        for a, b, c in zip(
                            cur_face,
                            edge_top_offset_out[(dir + 1) % 4],
                            dir_lookup[(dir + 1) % 4],
                        )
                    ]
                ) != (x, y):
                ...
                nx, ny = tuple(
                    [
                        ...
                        for a, b, c in zip(
                            next_face,
                            edge_top_offset_out[new_dir],
                            dir_lookup[(new_dir + 1) % 4],
                        )
                    ]
                )
            if grid[nx][ny] == "#":
